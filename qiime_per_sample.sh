...
case "${MARKER}" in
  ...
  ITS1)
    ...
    # Reverse complements used only for ITS1 Cutadapt trimming
    F_PRIMER_RC="TGATCCYTCCGCAGGT"
    R_PRIMER_RC="AACTTTYARCAAYGGATCTC"
...
awk 'BEGIN{FS="\t"} NR>1 && NF>0 {print $1}' "${METADATA}" |
while read -r SAMPLE_ID; do
  ...
  ###########################################################################
  # ITS1:
  # Remove forward/reverse primers AND reverse-complement primer sequences
  # from the opposite ends of reads.
  #
  # EF1 and ITS2:
  # Keep the original trimming command unchanged.
  ###########################################################################
  ...
  if ! qiime dada2 denoise-paired \
      ...
      --verbose > "${SAMPLE_DIR}/${SAMPLE_ID}-${MARKER}-dada2-output.txt" 2>&1
  then
  ...
  if ! qiime feature-table summarize \
      ...
      --m-sample-metadata-file "${SAMPLE_METADATA}" >/dev/null 2>&1
  then
  ...
  mv \
    "${SAMPLE_DIR}/exported_rep_seqs/dna-sequences.fasta" \
  ...
  mv \
    "${TMP_TREE_EXPORT}/dna-sequences.fasta" \
    "${TREE_FASTA}"
